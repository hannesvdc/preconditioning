...
def testbRecNet():
    ...
    net, _, _ = TrainRecursiveNet.setupRecNet(fixedA=True)
    precond_net, _, _ = TrainPreconditionedRecursiveNet.setupRecNet(fixedA=True)
...
def testAbRecNet():
    # Setup the network and load the weights
    net, _, _ = TrainRecursiveNet.setupRecNet(fixedA=False)
    precond_net, _, _ = TrainPreconditionedRecursiveNet.setupRecNet(fixedA=False)
    ...
    precond_weights = np.array([ 1.12513702 ,-1.85076988 , 0.5539859 , -2.07581943 , 1.32818009, -3.8991036,
                                -2.67406223 , 1.23153278, -2.26566653 ,-0.42730605]) # BFGS weights random A and b
    P = precond_net.P
    ...
    plt.xlim((-0.5, 10.5))
    plt.ylim((1.e-16, 70))
    plt.title(r'Random $b$, Random $A$, $P$ = ILU(0)')
    plt.legend()
    plt.show()
